...
class NZTM2000:
    ...
    @classmethod
    def latlng_k(cls, phi, lmb):
        """
        Obtain point scale factor from geographic lat, lng (NZGD2000 or WGS84)
        coordinates.
        ...
        omega = lmb - cls.lmb0
        term1 = omega2/2 * psi*cos_phi2
        term2 = omega4/24 * cos_phi4 * (4.0 * psi3 * (1 - 6*t2) + psi2 * (1 + 24*t2) - 4 * psi * t2)
        term3 = omega6/720 * cos_phi6 * (61 - 148*t2 + 16*t4)

        k = cls.k0 * (1 + term1 + term2 + term3)
        return k

    @classmethod
    def NZTM_k(cls, E, N):
        """
        Obtain point scale factor from NZTM2000 map coordinates.
        ...
        t_ = math.tan(phi_)
        E_ = E - cls.E0
        y = (E_*E_)/(cls.k0*cls.k0*rho_*nu_)
        ...
        term1 = y/2
        term2 = y2/24 * (4*psi_ * (1 - 6*t_2) - 3 * (1 - 16*t_2) - (24*t_2/psi_))
        term3 = y3/720
...
if __name__ == "__main__":
    ...
    print "%.8f"%NZTM2000.latlng_k(-34.0000, 173.0000)
    # expect 0.9996
    # got 0.99960000

    print "%.8f"%NZTM2000.latlng_k(-35.0000, 173.0000)
    # expect 0.9996
    # got 0.99960000

    print "%.8f"%NZTM2000.latlng_k(-36.0000, 173.0000)
    # expect 0.9996
    # got 0.99960000

    print "%.8f"%NZTM2000.NZTM_k(1600000.00,6237844.02)
    # expect 0.9996
    # got 0.99960000

    print "%.8f"%NZTM2000.latlng_k(-34.0000, 174.0000)
    print "%.8f"%NZTM2000.NZTM_k(*NZTM2000.latlng_to_NZTM(-34.0000, 174.0000))
    # expect identical
    # got 0.99970513
    # got 0.99970513

    print "%.8f"%NZTM2000.latlng_k(-35.0000, 174.0000)
    print "%.8f"%NZTM2000.NZTM_k(*NZTM2000.latlng_to_NZTM(-35.0000, 174.0000))
    # expect identical
    # got 0.99970263
    # got 0.99970263

    print "%.8f"%NZTM2000.latlng_k(-36.0000, 174.0000)
    print "%.8f"%NZTM2000.NZTM_k(*NZTM2000.latlng_to_NZTM(-36.0000, 174.0000))
    # expect identical
    # got 0.99970009
    # got 0.99970009

    print "%.8f"%NZTM2000.NZTM_k(1096802.28,4728776.8709)
    print "%.8f"%NZTM2000.latlng_k(*NZTM2000.NZTM_to_latlng(1096802.28,4728776.8709))
    # expect identical
    # got 1.00271317
    # got 1.00271317
